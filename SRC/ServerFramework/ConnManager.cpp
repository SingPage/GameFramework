...
VOID CConnectionManager::SignalHandler(INT32 nValue){
...
BOOL CConnectionManager::SetSignal(){

	FUNCTION_RUNNING

	m_NewAction.sa_handler = CConnectionManager::SignalHandler;

	sigemptyset(&m_NewAction.sa_mask);

	m_NewAction.sa_flags = 0;

	sigaction(SIGPIPE, &m_NewAction, &m_OldAction);
...
BOOL CConnectionManager::RestoreSignal(){

	FUNCTION_RUNNING

	sigaction(SIGPIPE, &m_OldAction, NULL);
